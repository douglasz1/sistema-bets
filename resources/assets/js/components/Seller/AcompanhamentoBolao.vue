<template>
  <div>
    <div class="btn-group btn-group-full p-tb">
      ...
      <button @click="buscarApostas" type="submit" class="btn btn-success p-t">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      buscarApostas: function () {
        this.$http.get('/seller/bolao/acompanhamento/relatorio', {
          params: {
            dataInicial: this.dataInicial,
            dataFinal: this.dataInicial
          }
        }).then(response => {
          response.json().then(res => this.apostas = res.apostas);
        }, error => console.log(error));
      },
      abrirBilhete: function (aposta) {

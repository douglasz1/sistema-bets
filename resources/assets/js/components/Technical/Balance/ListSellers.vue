  ...
  export default {
    ...
    computed: {
      filteredSellers() {
        ...
        const list = _.orderBy(this.sellers, 'manager.name', 'asc')
    ...
    methods: {
      getSellers () {
        this.$http.get('/technical/balance/sellers')
          .then(response => this.sellers = response.body.sellers.map(item => {
            item.save = false
            item.reset = false
            return item
          }))
          .catch(() => showToast('Erro ao listar operadores'))
      },
      saveSeller (seller) {
        seller.save = false

        this.$http.post('/technical/balance/send', {seller})
          .then(response => {
            ...
            var savedSeller = sellersave

            setTimeout(function () {
              savedSeller.save = false
            }, 300)
          })
          .catch(() => showToast('Erro ao salvar alteração!'))
      },
      resetBalance (seller) {
        this.$http.post('/technical/balance/reset', {seller})
          .then(response => {
            let responseSeller = response.body.seller
      ...
      resetAll (force = false) {
        const response = confirm('Você gostaria de reiniciar o saldo de todos?')
        if (response) {
          this.isLoading = true

          this.$http.post('/technical/balance/reset-all', {force: force})
            .then(response => {
              ...
              showToast('Saldo reiniciado com sucesso!')
            })
            .catch(() => {
              this.isLoading = false
      ...
      zeroToAll () {
        const response = confirm('Você gostaria de zerar todos os saldos?')
        if (response) {
          this.isLoading = true

          this.$http.post('/technical/balance/zero-all')
            .then(() => {
              this.isLoading = false
              ...
              showToast('Saldo zerado com sucesso!')
            })
            .catch(() => {
              this.isLoading = false
              showToast('Erro ao salvar alteração!')
...
<style scoped>
    ...
    .has-error .input-group-addon .fa-dollar:before,

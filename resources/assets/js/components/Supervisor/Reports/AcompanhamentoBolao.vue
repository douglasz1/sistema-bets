      ...
      <div class="table-responsive">
        <table class="table table-vcenter table-borderless table-condensed table-striped table-hover">
          ...
          <tr>
            ...
            <td>{{ quantidadeAbertos }}</td>
            <td>{{ quantidadePremiados }}</td>
            <td class="text-success"><b>{{ valorEntrada | formatMoney }}</b></td>
            <td>
              <span class="label label-danger">
                <b>{{ valorPremios | formatMoney }}</b>
              </span>
            </td>
            <td class="text-warning"><b>{{ valorComissoes | formatMoney }}</b></td>
            <td
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      buscarApostas() {
        this.$http
          .post('/supervisor/bolao/acompanhamento/busca', {apostaId: this.apostaId})
          .then(response => this.apostas = response.body.apostas)
      ...
      pegarApostas() {
        ...
        this.$http
          .get('/supervisor/bolao/acompanhamento/relatorio', {
            params: {
      ...
      getManagers(companyId) {
        this.$http.get('/supervisor/managers/pluck', {params: {companyId: companyId}})
          .then(response => {
      ...
      abrirAposta(aposta) {
        ...
        this.$refs.resumoBilheteBolao.buscarBilhete(aposta.id)
      },
      ...
      cancelarAposta(aposta) {
        this.$http.put(`/supervisor/bolao/acompanhamento/cancelar/${aposta.id}`)
          .then(() => {
    ...
    computed: {
      ...
      valorPremios () {
        const list = this.apostas;
        let value = _.sumBy(list, aposta => aposta.situacao === 'vencedor' ? parseFloat(aposta.prize) : 0);
        return parseFloat(value).toFixed(2);
      },
      valorComissoes () {
        const list = this.apostas;
        let value = _.sumBy(list, aposta => aposta.situacao !== 'cancelado' ? parseFloat(aposta.comissao) : 0);
        return parseFloat(value).toFixed(2);
      },
      quantidadePremiados () {
        const list = this.apostas;
        let winners = _.filter(list, aposta => aposta.situacao === 'vencedor');
      ...
      saldo () {
        let value = parseFloat(this.valorEntrada) - parseFloat(this.valorPremios) - parseFloat(this.valorComissoes);
      ...
      apostasOrdenadas() {
        ...
        return _.orderBy(this.apostas, aposta => parseFloat(aposta[`${ordernarPor}`]), ordemClassificacao)
      }
    },
    created() {

  ...
  <div class="row">
    <div class="col-lg-8 col-md-8 col-sm-8">
      ...
      <div class="row">
        ...
        <div
          v-show="loading"
          class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center loading">
...
<script>
  ...
  export default {
    components: {BetSimulate, ListLeagues, ListMatches, SportsSelect},
    data() {
      return {
    ...
    methods: {
      ...
      getSelectedLeague(leagueId) {
        ...
        if (leagueId !== '') {
          this.$http.get('/leagues/matches', {params: {league_id: leagueId}})
            .then(response => {
              response.json().then(res => {
      ...
      getLeaguesHaveMatches() {
        this.$http.get('/leagues').then(response => {
          response.json().then(res => {
            return this.leaguesHaveMatches = res.leagues;
          });
        }, error => {
          console.log(error);
        });
      ...
      searchLeagues() {
        if (this.matchFilter.length >= 3) {
          this.searching = true;
          this.$http.post('/leagues/search', {q: this.matchFilter})
            .then(response => {
              response.json().then(res => {
                this.selectedLeague = res.leagues;
